...
- **PR body format:** Summary (what problem, what the fix does); Changes (bullet list: file path + what changed); Testing; **Evidence it works**. Include "Fixes #66" (or Closes #66) in the body so GitHub auto-closes the issue on merge. End the body with `Signed-off-by: Deepak Jain <deepujain@gmail.com>`; NemoClaw's DCO workflow checks the PR description as well as commit trailers.
