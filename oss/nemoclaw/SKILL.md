...
- **Subprocess termination requires observed exit.** A successful `SIGTERM`/`SIGKILL` request or elapsed timeout does not prove the child was reaped. Do not discard the only `ChildProcess` handle until `exit`/terminal failure is observed. If bounded cleanup expires, return a distinct recoverable state that retains the handle and original failure, and test the no-event path.
...
- **Documentation Writer Review is exact-head evidence.** After every commit, merge-from-main update, conflict resolution, or maintainer push, rerun or refresh the documentation review and update its head/blob receipt metadata to the current PR head. Do this even when the final change is test-only: the receipt attests to the complete final change set, not only the latest file.
...
- **Quality Gates are reviewed content.** Fill tests/docs/sensitive-path/CI-waiver checkboxes with concrete justifications. Do not leave sensitive-path review, skipped CI, or docs warning state ambiguous.
...
- **Security scans apply to test fixtures too.** Avoid `existsSync(path)` followed later by a read or write of the same fixture path. Assert absence from one directory listing and assert presence with a direct read so CodeQL does not find a check-then-use filesystem race and the test still proves observable state.
