...
- **Strict snapshot failures must clean only their newly created backup.** When a strict pre-upgrade snapshot returns a timestamp path and then reports failure, remove only that path through the existing path-confined backup-removal boundary. A successful cleanup must remove its manifest from downstream evidence; a cleanup failure must preserve the original backup failure and add the exact cleanup diagnostic. Test successful cleanup, cleanup failure, and unchanged non-strict behavior ([#11589](https://github.com/NVIDIA/NemoClaw/pull/11589)).
...
- **Assert best-effort callback effects after the public call.** When production catches callback failures, an assertion thrown inside a test double can be swallowed and leave a false green. Record the observation in the callback, let the public lifecycle finish, then assert the recorded state outside the caught boundary. Merged #11506 used this pattern to prove channel shutdown held the portable-host fence.
...
- **Cache expensive patched fixtures as immutable sources, not shared case state.** Build the expensive fixture once only when its cached content remains immutable. Give every case an isolated recursive copy, rebind embedded managed paths to that case root, and retain suite-level cleanup. Benchmark the same focused workload before and after; when concurrency also changes, include a bounded shuffled run. Merged #11502 and #11545 used these boundaries to reduce suite time without sharing mutable homes or registries.
